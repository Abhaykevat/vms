...
// lib/screens/request_visit_screen.dart
import 'package:flutter/material.dart';
import 'package:provider/provider.dart';
import 'package:vms/providers/visit_request_provider.dart';

class RequestVisitScreen extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    final provider = Provider.of<VisitRequestProvider>(context);
    ...
    return Scaffold(
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            _buildLabel("Host Name"),
            _buildTextField(provider.hostController),
            SizedBox(height: 16),

            _buildLabel("Company Name"),
            _buildDropdown(provider.companies, provider.selectedCompany, provider.setCompany),
            SizedBox(height: 16),

            _buildLabel("Purpose of Visit"),
            _buildDropdown(provider.purposes, provider.selectedPurpose, provider.setPurpose),
            SizedBox(height: 16),

            _buildLabel("Date"),
            _buildDateTimeField(provider.dateController, Icons.calendar_today, () => provider.selectDate(context)),
            SizedBox(height: 16),

            _buildLabel("Time"),
            _buildDateTimeField(provider.timeController, Icons.access_time, () => provider.selectTime(context)),
            SizedBox(height: 30),
            ...
            SizedBox(
              ...
              height: 50,
              child: ElevatedButton(
                onPressed: provider.isLoading ? null : () => provider.sendVisitRequest(context),
                style: ElevatedButton.styleFrom(
                  backgroundColor: Color(0xFF00C27C),
                  shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(30)),
                ),
